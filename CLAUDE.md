...
## Last session recap — 2026-06-16 (session 17)
Replaced hard 10-message context window with LangGraph-native stateful history and rolling summarization.

Key changes:
- `graph.py`: `ChatState` now has a `summary: str` field (persisted by `AsyncPostgresSaver`). New `maybe_summarize` node fires at `START` of every user turn: when `len(messages) > 20`, it finds a safe `HumanMessage` boundary, summarizes older messages via LLM (folding in any prior summary), removes them with `RemoveMessage`, and stores the new summary. `agent` now accepts `config: RunnableConfig`, fetches hot memories from DB directly, and prepends a fresh `SystemMessage` (base + hot memories + summary) on every LLM call. Graph: `START → maybe_summarize → agent`; tool loops bypass `maybe_summarize`. Constants: `SUMMARIZE_THRESHOLD = 20`, `KEEP_RECENT = 8`.
- `routers/chat.py`: checkpointer-stateful seeding — if the thread already has checkpoint state, only the new `HumanMessage` is injected; otherwise all messages are loaded from DB (ASC) for initial seeding. System-prompt/hot-memories construction removed from `chat.py`.
...
- LangGraph graph: `START → maybe_summarize → agent → tools_router → [hitl | read_tools | END]`; `hitl` and `read_tools` edge back to `agent` (bypassing `maybe_summarize`). `maybe_summarize` compacts history when `len(messages) > 20`, storing a rolling `summary` in `ChatState`. `hitl_node` intercepts write tool calls, interrupts for user approval (LLM-generated summary), then executes writes directly on approval.
...
- `graph.py`: `build_graph(checkpointer=None)` factory; `ChatState` (`messages: Annotated[list, add_messages]`, `summary: str`); `SUMMARIZE_THRESHOLD = 20`, `KEEP_RECENT = 8`; `maybe_summarize` node — summarizes oldest messages when count > threshold, stores result in `summary`, removes old messages via `RemoveMessage`; `agent(state, config)` — fetches hot memories from DB, prepends `SystemMessage` (base + memories + summary) on every LLM call; tools use `RunnableConfig` for db access via `config["configurable"]["db"]`; read tools: `search_database`, `search_web` (Tavily), `retrieve_memories`, `find_memory` (returns `[ID: uuid] content` lines) — all tool descriptions instruct rich keyword-dense queries; write tools: `save_memory`, `delete_memory(memory_id, content)`, `update_memory(memory_id, old_content, new_content)` — validate UUID format before DB call, return "Memory not found." on 0 rows; `hitl_node` gates all write calls with `interrupt({"summary": llm_text})`, resumes with `{"action": "approve"|"deny"|"replan", "message"?: str}` (legacy bool supported); if any read tool is mixed into the same AIMessage as write tools, all call IDs get error ToolMessages and interrupt is skipped; "replan" appends a `HumanMessage` so agent re-plans; `_summarize_write_calls` serializes pending ops as JSON and asks LLM for a plain-English summary; `_WRITE_TOOL_MAP` dispatches approved calls; `tools_router` conditional edge; `_extract_keywords` via `llm.with_structured_output(_Keywords)`
- `db.py`: `chunk_text(text)`, `save_chunks(db, collection, content, metadata, keywords=[])`, `save_memory(db, type, content) → str`, `get_hot_memories(db) → list[str]`, `save_memory_embedding(db, memory_id, type, content, keywords)`, `delete_memories(db, memory_ids) → int`, `update_memory(db, memory_id, new_content, keywords) → int` (returns rows updated), `encode_cursor(*parts)`, `decode_cursor(cursor)`
- `rag.py`: `_llm_score(query, rows) → list[float]` — structured-output LLM scoring with a defined 0–10 scale and semantic-category guidance; falls back to RRF on error. `search_database_impl(query, limit, db)` — embed → hybrid search → `_llm_score` → filter ≥ 7 → batch-fetch; returns `list[str]`. `search_memories_impl(query, limit, db)` — hybrid search + `_llm_score`, filter ≥ 7, returns content strings. `search_memories_with_ids_impl(query, limit, db)` — hybrid search + `_llm_score`, filter ≥ 5, returns `list[{memory_id, content}]`
